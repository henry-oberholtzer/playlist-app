...
const storageReference = (directory) => {
		...
		return (filename) => {
			return (file) => {
				...
				uploadBytes(storageRef, file).then((snapshot) => {
					getDownloadURL(snapshot.ref).then((url) => callbackFunction(url));
				});
			};
		};
	};
...
const FileUpload = (props) => {
	...
	const handleFileUpload = (e) => {
		...
		if (!uploadedFile) {
			fileError(
				`Please select a valid ${props.fileTypes} file before uploading`
			);
		} else if (uploadedFile.size > props.maxMB * 1048576) {
			fileError(`Please choose a file less than ${props.maxMB} MB`);
		} else if (
			!fileMIMEtypes[props.fileTypes].includes(uploadedFile.type) &&
			!uploadedFile.name.match(fileRegex[props.fileTypes])
		) {
			...
			console.log(name);
			const newFile = new File([uploadedFile], name, {
				type: uploadedFile.type,
			});
			imgRef(props.callbackFunction)(name)(newFile);
		}
	};
